...
fetch('../text/drinks.json')
    ...
    .then(function(data) {
        for (let rows = 0; rows < data.length; rows++) {
            DrinksID[rows] = data[rows].DrinksID
            Drinks[rows] = data[rows].Drinks
            DrinksCategory[rows] = data[rows].DrinksCategory
        }
    })
...
fetch('../text/member.json')
    ...
    .then(function(data) {
        for (let rows = 0; rows < data.length; rows++) {
            MemberID[rows] = data[rows].MemberID
            MemberPassword[rows] = data[rows].MemberPassword
            BirthMonth[rows] = data[rows].BirthMonth
            MemberPoints[rows] = data[rows].MemberPoints
            Coupon[rows] = data[rows].Coupon
        }
    })
fetch('../text/food.json')
    ...
    .then(function(data) {
        for (let rows = 0; rows < data.length; rows++) {
            FoodID[rows] = data[rows].FoodID
            FoodName[rows] = data[rows].FoodName
            FoodCategory[rows] = data[rows].FoodCategory
        }
    })
fetch('../text/ingredients.json')
    ...
    .then(function(data) {
        for (let rows = 0; rows < data.length; rows++) {
            IngredientsID[rows] = data[rows].IngredientsID
            Ingredients[rows] = data[rows].Ingredients
            Quantity[rows] = data[rows].Quantity
        }
    })
